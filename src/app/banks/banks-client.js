      ...
      <div style={{ padding: '0 20px 16px' }}>
        ...
        {expanded && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '14px' }}>
            ...
            {/* Security Check: Only display credentials to logged in agents/admins */}
            {hasUser && userRole && userRole !== 'user' && !bank.direct_submit && (() => {
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              padding: '10px 0',
              borderRadius: 'var(--border-radius-sm)',
              background: (!hasUser || affiliateLink || userRole === 'user') ? 'var(--gradient-primary)' : 'rgba(99, 102, 241, 0.15)',
              border: (!hasUser || affiliateLink || userRole === 'user') ? 'none' : '1px solid rgba(99, 102, 241, 0.3)',
              color: (!hasUser || affiliateLink || userRole === 'user') ? '#fff' : 'var(--color-primary)',
              fontWeight: 700,
              fontSize: '13px',
              cursor: 'pointer',
              letterSpacing: '0.02em',
            ...
            onMouseEnter={e => {
              if (!hasUser || affiliateLink || userRole === 'user') {
          ...
          >
            {hasUser ? ((affiliateLink || userRole === 'user') ? 'Apply Now' : 'Apply for Client') : 'Check Eligibility'}
          </button>
        </div>
      </div>
    </div>
...
// ─── Main Page ───────────────────────────────────────────────────────────────
export default function BanksClient({ defaultCategory = 'ALL' }) {
  const router = useRouter();
  ...
  const handleApplySubmit = async (e) => {
    ...
    try {
      ...
      if (affiliateLink) {
        ...
        window.open(affiliateLink, '_blank', 'noopener,noreferrer'); // Security (F3): prevent tabnapping
      } else {
          ...
          <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto' }}>
            <div style={{ marginBottom: '8px' }}>
              <span style={{ fontSize: '12px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-primary)', background: 'rgba(45,212,191,0.1)', padding: '4px 14px', borderRadius: '99px' }}>
                {user ? (userRole === 'user' ? 'Customer Portal' : 'Agent Portal') : 'Partner Network'}
              </span>
            </div>
            <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(1.75rem, 4vw, 2.75rem)', fontWeight: 800, color: 'var(--color-text-primary)', marginBottom: '12px', lineHeight: 1.15 }}>
              All Partner Banks & NBFCs
            </h1>
            <p style={{ color: 'var(--color-text-secondary)', fontSize: 'var(--text-base)', maxWidth: '560px', lineHeight: 1.6 }}>
              Browse all available bank policies, check pincode serviceability, and search lending requirements.
            </p>
